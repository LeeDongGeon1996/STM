...
public class UploadController {
	 ...
	 * Map returnObject = new HashMap();
	 * 
	 * try { // multiparthttpservletrequest 생성 MultipartHttpServletRequest mhsr =
	 ...
	 * ArrayList();
	 * 
	 * // 디렉토리가 없다면 생성한다 File dir = new File(path); if (!dir.isDirectory()) {
	 * dir.mkdirs(); }
	 * 
	 * // 값이 나올때까지 while (iter.hasNext()) { fieldName = (String) iter.next(); mfile
	 * = mhsr.getFile(fieldName); String origName;
	 * 
	 * origName = new String(mfile.getOriginalFilename().getBytes("8859_1"),
	 * "UTF-8");// 한글 깨짐방지 코드
	 * 
	 * // 파일명이 없다면 if ("".equals(origName)) { continue; }
	 * 
	 * // 파일 명 변경 (uuid로 암호화) String ext =
	 * origName.substring(origName.lastIndexOf('.')); String saveFileName =
	 * getUuid() + ext;
	 * 
	 * // 설정한 path에 파일저장 File serverFile = new File(path + File.separator +
